//-----------------------------------------------------------------------------
// \file    test_mmcsd.c
// \brief   implementation of OMAP-L138 mmc/sd test.
//
//-----------------------------------------------------------------------------
...
uint32_t TEST_mmcSD(void)
{
   ...
   uint32_t i;
   uint32_t rca;
   uint8_t num_read_attempts = 0;
   uint8_t num_write_attempts = 0;

   printf("------------------------------------------------------------\r\n");
   printf("                     OMAP-L138 MMCSD Test\r\n\r\n");
...
INIT:
   ...
   if (rtn != ERR_NO_ERROR)
   {
      if (num_read_attempts++ < 2)
      {
         printf("\tMMCSD Status Regs: 0x%02X 0x%04X 0x%02X\r\n", MMCSD0->MMCCLK, MMCSD0->MMCST0, MMCSD0->MMCST1);
   ...
   if (rtn != ERR_NO_ERROR)
   {
      if (num_write_attempts++ < 2)
      {
         printf("\tMMCSD Status Regs: 0x%02X 0x%04X 0x%02X\r\n", MMCSD0->MMCCLK, MMCSD0->MMCST0, MMCSD0->MMCST1);
         printf("\terror writing sd memory...try again\r\n");
         goto INIT;
      }
      printf("\tMMCSD Status Regs: 0x%02X 0x%04X 0x%02X\r\n", MMCSD0->MMCCLK, MMCSD0->MMCST0, MMCSD0->MMCST1);
      printf("\terror writing sd memory...too many retries!\r\n");
      return (rtn);
   }
   else
   {
